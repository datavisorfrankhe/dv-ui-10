...
$(function() {
  ...
  //update, as pwd is pssed in in url already
  if (db_host.indexOf('basic_user=') >= 0) {
      var regex = /basic_user=([^&]+)/g
      var match = regex.exec(db_host)
      basic_user = decodeURIComponent(match[1]);
      if(db_host.indexOf('password=') >=0 ){
          var regex1 = /password=([^&]+)/g
          var match1 = regex1.exec(db_host)
          host_password =  decodeURIComponent(match1[1]);
          console.log(db_host, basic_user, host_password)
          init();
          return;
      }
  }
...
function query(key, str, callback) {
  ...
  var isCreate = str.toLowerCase().indexOf('create table') >= 0 && str.toLowerCase().indexOf('show create table') < 0;
  var isDrop = str.toLowerCase().indexOf('drop table') >= 0;
  var isInsert = str.toLowerCase().indexOf('insert into') >= 0;
  var isSelect = str.toLowerCase().indexOf('select') >= 0;
